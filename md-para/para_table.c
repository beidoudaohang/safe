...
s8 one_para_adr_read_processing(const u16 adr, para_stream *ps)
{
...
#endif
	cnt = ptable[adr_index].link_para_a[mod_index].link_para_t.para_type_t.dat;

	para_type_len = get_para_type_len(ptable[adr_index].link_para_a[mod_index].link_para_t.para_type_t);
	...
	if (!para_type_len) {
		...
		_para->para_adr = adr;
		_para->para_len = ptable[adr_index].link_para_a[mod_index].link_para_t.len;
		_para->para_type_t.st_t.st = 0;
		_para->para_type_t.st_t.type = (u8)(0x000f & cnt);
	}
	//read dat
	//RLDEBUG("one_para_adr_read_processing:begin read para,para len is:%d\r\n", ptable[adr_index].link_para_a[mod_index].link_para_t.len);
	ps->next = ps->next + sizeof(para);
	if (NULL != (ptable[adr_index].link_para_a[mod_index].link_para_t.paradeal)) {
		parafun = (ptable[adr_index].link_para_a[mod_index].link_para_t.paradeal);
		para_num = parafun((ptable[adr_index].link_para_a[mod_index].link_para_t.dat), (ps->next), (ptable[adr_index].link_para_a[mod_index].md_adr), (ptable[adr_index].dig_adr), PARA_RD);
		if (para_num < 0) {
			//err = ((0xf0 & (PARA_ADR_OTHER_ERR << 4))|(0x0f&((para_table_a[adr_index].link_para_a[mod_index].link_para_t.para_type_t.dat))));
			SET_PARA_ERR_CODE(err, PARA_ADR_OTHER_ERR);
			SET_PARA_TYPE(err, (ptable[adr_index].link_para_a[mod_index].link_para_t.para_type_t.dat));
			goto ONE_PARA_ADR_READ_ERR;
		}
	} else {
		memcpy((void*)(ps->next), (void*)(ptable[adr_index].link_para_a[mod_index].link_para_t.dat), (ptable[adr_index].link_para_a[mod_index].link_para_t.len));
	}
	ps->next = ps->next + (ptable[adr_index].link_para_a[mod_index].link_para_t.len);
	ps->paralen += (sizeof(para) + (ptable[adr_index].link_para_a[mod_index].link_para_t.len));
...
s8 one_para_adr_set_processing(const s8 *src, para_stream *ps)
{
	...
	//check para len
	cnt = ptable[adr_index].link_para_a[mod_index].link_para_t.len;
	...
	if (NULL != (ptable[adr_index].link_para_a[mod_index].link_para_t.paradeal)) {
		...
		for (cnt = 0; cnt < (_para->para_len / para_type_len); cnt++) {
			...
			if (check_para_range(&(ptable[adr_index].link_para_a[mod_index].link_para_t), (void*)(src + sizeof(para) + cnt * para_type_len))) {
		...
		parafun = (ptable[adr_index].link_para_a[mod_index].link_para_t.paradeal);
		para_num = parafun((void*)(ptable[adr_index].link_para_a[mod_index].link_para_t.dat), (void*)(src + sizeof(para)), (ptable[adr_index].link_para_a[mod_index].md_adr), (ptable[adr_index].dig_adr), PARA_RW);
		if (para_num < 0) {
		...
		/*最大最小值判断*/
		if (check_para_range(&(ptable[adr_index].link_para_a[mod_index].link_para_t), (void*)(src + sizeof(para))))
		{
			RLDEBUG("one_para_adr_set_processing:para range err\r\n");
			SET_PARA_ERR_CODE(err, PARA_OVERRANGE);
			SET_PARA_TYPE(err, _para->para_type_t.dat);
		...
		/*复制参数*/
		memcpy((void*)(ptable[adr_index].link_para_a[mod_index].link_para_t.dat), (void*)(src + sizeof(para)), (ptable[adr_index].link_para_a[mod_index].link_para_t.len));
	}

	if (ptable[adr_index].dig_adr) {
		set_adr_add(ptable[adr_index].dig_adr , ptable[adr_index].link_para_a[mod_index].md_adr);
	}
	ps->next = ps->next + (ptable[adr_index].link_para_a[mod_index].link_para_t.len);
...
ONE_PARA_ADR_SET_ERR:
	...
	*(u16*)(ps->next) = _para->para_adr;	//para adr
	...
	if ((PARA_ADR_INVALID == GET_PARA_ERR_CODE(err)) || (PARA_ADR_OTHER_ERR == GET_PARA_ERR_CODE(err))) {
		...
		*(u8*)(ps->next) = err | (ptable[adr_index].link_para_a[mod_index].link_para_t.para_type_t.dat);		//para err
	}
	ps->next = ps->next + 1;
	if (PARA_LEN_ERR == GET_PARA_ERR_CODE(err))
		*(u16*)(ps->next) = (ptable[adr_index].link_para_a[mod_index].link_para_t.len);
	else
		*(u16*)(ps->next) = 0;
	ps->next = ps->next + 2;
